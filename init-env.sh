...
DORISWRITER_DIR=$DATAX_EXTENSION_HOME/selectdbwriter
...
if [ ! -d $DATAX_GIT_DIR ]; then
    ...
    ln -s $DORISWRITER_DIR $DATAX_GIT_DIR/selectdbwriter
else
...
if [ `grep -c "selectdbwriter" $DATAX_POM` -eq 0 ]; then
    echo "No selectdbwriter module in $DATAX_POM, add it"
    if [[ USE_MACOS -eq 0 ]];then
        cp $DATAX_POM ${DATAX_POM}.orig
        sed -i "s/<\/modules>/    <module>selectdbwriter<\/module>\n    <\/modules>/g"  $DATAX_POM
    else
        sed -i '.orig' "s/<\/modules>/    <module>selectdbwriter<\/module>\n    <\/modules>/g"  $DATAX_POM
    fi
else
    echo "selectdbwriter module exists in $DATAX_POM"
fi

if [ `grep -c "selectdbwriter" $DATAX_PACKAGE` -eq 0 ]; then
    echo "No selectdbwriter module in $DATAX_PACKAGE, add it"
    if [[ USE_MACOS -eq 0 ]];then
    ...
    echo "selectdbwriter module exists in $DATAX_PACKAGE"
fi
